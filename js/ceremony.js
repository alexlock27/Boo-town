  ...
  function openSequence() {
    ...
    function reveal() {
      ...
      const card = el('div', { class: 'reveal-card ' + glowClass }, [
        el('div', { class: 'reveal-art', html: renderItem(result.item, { size: 180, cls: result.item.fx ? '' : 'art-idle' }) }),
      ...
      if (result.duplicate) {
        ...
        const key = result.rarity === 'secret' ? 'boxSecret' : result.rarity === 'ultra' ? 'boxUltra' : result.rarity === 'rare' ? 'boxRare' : 'boxCommon';
        guideBubble.textContent = guideLine(key);
        speakMaybe(guideBubble.textContent);
        buttons.appendChild(el('button', { class: 'btn big', text: 'Put it in my town 🏡', onclick: () => { sfx.tap(); ctx.go('town', { place: result.item.id, from: 'ceremony' }); } }));
