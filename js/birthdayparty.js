// Twin1 & Twin2's Twin Party Garden — a self-contained, saved birthday celebration.
...
export const PARTY_CONFIG = {
  twin1: {
    name:'TWIN1', colour:'#FF72C6', deep:'#8A3D91', soft:'#FFE0F4',
    icon:'⭐', booId:'boo_birthday_twin1', booName:'Twin1 Starshine',
    booLine:'Crown bright, starry and made only for Twin1.',
    cake:'Strawberry Star Cake', cakeIcon:'🍓', theme:'Starshine Palace'
  },
  twin2: {
    name:'TWIN2', colour:'#28C9C1', deep:'#2566A8', soft:'#D9FBF8',
    icon:'🚀', booId:'boo_birthday_twin2', booName:'Twin2 Turbo',
    booLine:'Fast, musical and made only for Twin2.',
...
export function mount(container, params, ctx) {
  music.play('fair');
  mutate(st => {
    st.birthdayParty = st.birthdayParty || { opened:{twin1:false,twin2:false}, visits:0 };
    st.birthdayParty.opened = st.birthdayParty.opened || { twin1:false,twin2:false };
  ...
  const zones = {};
  const timers = [];
  const partyMoments = { twin1:new Set(), twin2:new Set() };
  ...
  function refreshFinale() {
    ...
    const ready = opened.twin1 && opened.twin2;
    finale.disabled = !ready;
    finale.classList.toggle('ready', ready);
    finale.querySelector('small').textContent = ready ? 'Both Birthday Boos are ready!' : `${Number(opened.twin1)+Number(opened.twin2)} of 2 presents opened`;
  }

  function showFinale() {
    const opened = getState().birthdayParty.opened;
    if (!(opened.twin1 && opened.twin2)) return;
    sfx.fanfare();
    ...
    const card = el('div', { class:'twin-finale-card' }, [
      ...
      el('button', { class:'btn finale-again', text:'Celebrate again!', onclick:() => { overlay.remove(); celebrateParty('twin1'); celebrateParty('twin2'); } }),
      el('button', { class:'btn soft', text:'Back to the parties', onclick:() => overlay.remove() })
    ]);
    overlay.appendChild(card); document.body.appendChild(overlay);
    requestAnimationFrame(() => overlay.classList.add('show'));
    speakMaybe('Happy eleventh birthday, Twin1 and Twin2!');
  }
