...
export function mount(container, params, ctx) {
  ...
  // The daily-hider fallback is a real, always-present Meadow oak — never an invisible
  // coordinate. It is scenery rather than a placed item, so it cannot consume inventory
  // space or be moved/removed, but it uses the same art, depth and hidePoint as an oak.
  function renderFallbackHideOak() {
    if (AREA.key !== MEADOW_OAK_FALLBACK.zone) return;
    const hp = HIDE_POINTS[MEADOW_OAK_FALLBACK.item];
    if (!hp) return;
    const row = hp.row != null ? hp.row : 1;
    const size = (ACT_SIZE[MEADOW_OAK_FALLBACK.item] || 92) * ROW_SCALE[row];
    const oak = resolveItem(MEADOW_OAK_FALLBACK.item);
    if (!oak) return;
    const wrap = el('div', { class: 't-hide-fallback-oak', dataset: { item: MEADOW_OAK_FALLBACK.item, zone: MEADOW_OAK_FALLBACK.zone } });
    wrap.style.left = (clamp01(MEADOW_OAK_FALLBACK.x) * zoneW - size / 2) + 'px';
    wrap.style.top = (viewH * ROW_GROUND[row] - size + 8) + 'px';
    wrap.style.zIndex = String(Math.round(viewH * ROW_GROUND[row]));
    wrap.innerHTML = renderItem(oak, { size });
  ...
  if (typeof window !== 'undefined') {
    ...
    window.__townLife = {
      ...
      hideItemBBox: () => { const h = currentHide(); if (!h) return null; const n = [...ground.querySelectorAll('.t-item')].find(w => w.dataset.item === h.spot.item && w.dataset.zone === h.spot.zone) || ground.querySelector(`.t-hide-fallback-oak[data-item="${h.spot.item}"][data-zone="${h.spot.zone}"]`); return n ? n.getBoundingClientRect() : null; },
