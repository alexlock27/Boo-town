...
> split into `brainhelpers.js`). **F-07** child PII — CLOSED, and closed in full: the feature
> was retired and the tracked tree cleaned under Q1 (permanent grep-guard in place), and on
> 11 Aug 2026 the repository history itself was rewritten so the earlier commits no longer
> carry the content either. Nothing here needs re-checking, and this file no longer records
> where it used to sit — that pointer outlived its purpose the moment the history was clean,
> and a closed privacy finding should not double as a map (edit noted rather than made
> silently: F-07's row in §2 was generalised at the same time, RUN21v3).
...
| F-07 | S2 | M | **Child PII on a public site.** A personalised birthday-gift feature committed real first names and an age string into several tracked source files, which were then served live via GitHub Pages. Flagged for a maintainer decision rather than unilaterally rewritten, because it was **intentional personalised content**. *(CLOSED — see the status note at the top. The original row enumerated the exact files and save keys; that detail is deliberately not reproduced now the finding is closed, since its only remaining use would be as a map. RUN21v3.)* | Neutralise identifiers/keys and display copy, with a **lossless v12 save migration** for the affected save keys and inventory ids. Out of this audit's product-scope. |
