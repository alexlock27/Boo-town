...
await page.evaluate(() => localStorage.setItem('bootown.save.v1', JSON.stringify({ version: 1, name: 'Maya', guide: { body: 'sunshine', patch: 'cocoa', acc: 'bow', name: 'Twiggy' }, settings: { sound: true, music: true, voice: true } })));
...
const resetBtnDisabledBefore = await page.getAttribute('.btn.danger', 'disabled');
assert(resetBtnDisabledBefore !== null, 'reset button disabled until RESET typed');
await page.fill('.gu-danger .text-input', 'RESET');
await page.waitForTimeout(100);
const resetBtnDisabledAfter = await page.getAttribute('.btn.danger', 'disabled');
